export default class gUtility {
  ...
  static getVector(str, x, y, z) {
    if (str !== undefined)
      if (str !== '') {
        ...
        x = gUtility.getNumberOrDefault(parts[0], x);
        y = gUtility.getNumberOrDefault(parts[1], y);
        z = gUtility.getNumberOrDefault(parts[2], z);
      }
  ...
  static path(obj, is, value) {
    try {
      if (typeof is == 'string')
        return gUtility.path(obj, is.split('.'), value);
      else if (is.length == 1 && value !== undefined)
        return obj[is[0]] = value;
      else if (is.length == 0)
        return obj;
      else
        return gUtility.path(obj[is[0]], is.slice(1), value);
    } catch (e) {
  ...
  static HexToRGB(hex) {
    var r = gUtility.HexToR(hex) / 255;
    var g = gUtility.HexToG(hex) / 255;
    var b = gUtility.HexToB(hex) / 255;
    if (typeof window !== "undefined" && window.BABYLON)
      return new BABYLON.Color3(r, g, b);
  ...
  static HexToR(h) {
    return parseInt((gUtility.CutHex(h)).substring(0, 2), 16)
  }
  static HexToG(h) {
    return parseInt((gUtility.CutHex(h)).substring(2, 4), 16)
  }
  static HexToB(h) {
    return parseInt((gUtility.CutHex(h)).substring(4, 6), 16)
  }
  static CutHex(h) {
  ...
  static getGPSDiff(lat1, lon1, lat2, lon2) {
    let distance = gUtility.__distanceGPSMeters(lat1, lon1, lat2, lon2);
    let horizontal = gUtility.__distanceGPSMeters(lat1, lon1, lat2, lon1);
    let vertical = gUtility.__distanceGPSMeters(lat1, lon1, lat1, lon2);
  ...
  static angleDeg(angle, d = 0) {
    ...
    if (d !== 0)
      d = gUtility.angleDeg(d);

    if (angle.toLowerCase().indexOf('deg') !== -1) {
      angle = angle.toLowerCase().replace('deg', '');
      angle = gUtility.getNumberOrDefault(angle, d) * Math.PI / 180.0;
    } else {
      angle = gUtility.getNumberOrDefault(angle, d);
    }
    return angle;
  }
}
